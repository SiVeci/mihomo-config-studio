...
- v0.1.0 范围内不受影响：内置 Bundle 不走网络验签路径。真正受影响的是
  ...
  当前无法预判）。**待兑现项已关闭（v0.6.0 #10，[ADR-028](./ADR-028-pure-js-ed25519-backend.md)）**：
  纯 JS 回退（`@noble/ed25519`）已落地，`Medium_Phone_API_35`（本 ADR 未
  覆盖的第三台镜像，Chrome 124，同样落在 110–137 的"已实现但未默认开启"
  区间）上真机复验了完整的检查更新→安装→回滚链路，先确认该设备的
  `crypto.subtle` 确实不支持 Ed25519，再确认安装能走通——证明纯 JS 后端
  真的接住了这个缺口，不再需要"确认目标用户群 WebView 分布"这条备选路径。
