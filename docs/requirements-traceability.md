...
| NFR-SEC-04            | Todo               | M0-4；算法与密钥托管已定（ADR-010）。Android WebView 实测不支持 Ed25519 WebCrypto（`Medium_Phone_API_29`/`V14_API36_Large` 均失败），验签后端策略见 [ADR-013](./adr/ADR-013-ed25519-verifier-backend.md)：走可插拔 `Ed25519Verifier` 端口，v0.1.0 只接 SubtleCrypto 实现                                                                                                                                                                             |
