...
v1.0.0 版本文档的主题是"发布阻断项清零"，本片（#13）用这份文件直接核对，
没有重新翻一遍 PRD 和 CI 配置。**结果与 v0.9.0 收口时一致，仍是 4/5**：
第 1-4 条继续由 CI 机器回答（本轮真实 CI [33992350113](https://github.com/SiVeci/mihomo-config-studio/actions/runs/33992350113)
复核过，均通过）；第 5 条（Android 保存/重新打开可靠性）仍未进 CI，决策
H3 未变。**这不是本片能顺带解决的缺口，是记录在案、需要重开决策 H3 才能
改变的既定代价**，如实记为 v1.0.0 退出条件 3 的 Partial，不宣告已清零。
